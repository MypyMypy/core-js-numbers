...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  return (Math.abs(x1 - x2) ** 2 + Math.abs(y2 - y1) ** 2) ** (1 / 2);
}
...
function toNumber(value, def) {
  ...
  return typeof updateValue === 'number' && !Number.isNaN(updateValue)
    ? updateValue
    : def;
}
...
function getFibonacciNumber(index) {
  const numFiboArray = [0, 1];
  for (let i = 1; i <= index; i += 1) {
    const currentNum = numFiboArray[i];
    const prevNum = numFiboArray[i - 1];
    numFiboArray.push(currentNum + prevNum);
    if (i === index) {
      return numFiboArray[index];
    }
  }
  return 0;
}
 ...
 */
function isNumber(n) {
  return !(Number.isNaN(n) || typeof n !== 'number' || !Number.isFinite(n));
}
...
function getSumOfNumbers(...args) {
  return [...args].reduce((total, n) => total + n, 0).toFixed(1);
}
...
function getCountOfOddNumbers(n) {
  return n >= 0 ? Math.abs(Math.ceil(n / 2)) : Math.abs(Math.floor(n / 2));
}
